...
public class ReadStatusController {
  ...
  public ResponseEntity<SuccessResponse<List<ReadStatusResponseDto>>> findAllByUserId(
      @Parameter(name = "userId", description = "조회할 User ID") @RequestParam(value = "userId") UUID userId) {
    return ResponseEntity.status(HttpStatus.OK)
        .body(SuccessResponse.success(readStatusService.findAllByUserId(userId)));
  }

  @RequestMapping(method = RequestMethod.POST)
  @Operation(summary = "Message 읽음 상태 생성", operationId = "create_1")
  public ResponseEntity<SuccessResponse<ReadStatusResponseDto>> createReadStatus(
      @RequestBody ReadStatusPostDto readStatusPostDto) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(SuccessResponse.success(readStatusService.create(readStatusPostDto)));
  }

  @RequestMapping(value = "/{readStatusId}", method = RequestMethod.PATCH)
  @Operation(summary = "Message 읽음 상태 수정", operationId = "update_1")
  public ResponseEntity<SuccessResponse<ReadStatusResponseDto>> updateReadStatus(
      @Parameter(name = "readStatusId", description = "수정할 읽음 상태 ID") @PathVariable UUID readStatusId) {
    return ResponseEntity.status(HttpStatus.OK)
        .body(SuccessResponse.success(readStatusService.update(readStatusId)));
  }
